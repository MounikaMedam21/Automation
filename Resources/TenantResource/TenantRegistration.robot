...
Resource  ../../PageObjects/TenantObjects/TenantRegistration.robot
*** Keywords ***
...
Fill Tenant Registration Form
    [Arguments]   ${FullName}   ${DomainName}   ${Line1}    ${Line2}    ${Line3}    ${City}  ${State}   ${Country}   ${About}    ${FirstName}    ${LastName}   ${Email}    ${SupportNum}   ${Dropdown}
    ...
    ${mobile}=    Evaluate    '+91' + ''.join(random.choice('6789') + ''.join(random.choice('0123456789') for i in range(9)))    random
    [Return]   ${mobile}
